...
export function FileTabs({ files, activeFileId, isDark, onTabClick, onTabClose }: FileTabsProps) {
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-2 border-b overflow-x-auto ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}>
      {files.map(file => (
        <div
          key={file.id}
          className={`group flex items-center gap-2.5 px-4 py-2.5 rounded-xl cursor-pointer transition-all duration-200 hover:scale-105 active:scale-95 flex-shrink-0 ${
            activeFileId === file.id
              ...
              : (isDark ? 'bg-gray-700/50 text-gray-300 hover:bg-gray-700 hover:shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 hover:shadow-sm')
          }`}
        ...
        >
          ...
          <button
            ...
            className={`opacity-0 group-hover:opacity-100 p-1 rounded-lg hover:bg-white/20 transition-all duration-200 ${
              activeFileId === file.id ? 'opacity-70' : ''
            }`}
          >
